...
3. The sub-agent reports results via `send_agent_message` back to you
4. You can check on sub-agents with `subagent_list()`
5. Sub-agents auto-destroy after 10 minutes of idle. You can also destroy them early with `subagent_destroy(sub_agent_id)`
...
- **Sub-agent IDs**: They follow the pattern `your_id_sub_N` (e.g., `linus_sub_1`). You'll need these for `subagent_destroy` and `send_agent_message`.
