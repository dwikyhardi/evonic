...
class TestSchedulerActions:
    ...
    # ------------------------------------------------------------------
    # user_notification action tests
    # ------------------------------------------------------------------

    def test_action_user_notification_success(self):
        from backend.scheduler import scheduler
        config = {
            'channel_id': 'ch-1',
        ...
        with patch('backend.channels.registry.channel_manager') as mock_cm:
            mock_channel = MagicMock()
            mock_channel.is_running = True
            mock_cm.get_channel_instance.return_value = mock_channel
            sent = scheduler._action_user_notification(config)
            assert sent is True
            mock_cm.get_channel_instance.assert_called_once_with('ch-1')
            mock_channel.send_message.assert_called_once_with('user-42', 'Reminder: standup!')

    def test_action_user_notification_channel_not_found(self):
        from backend.scheduler import scheduler
        config = {
            'channel_id': 'ch-dead',
            'external_user_id': 'user-42',
        ...
        with patch('backend.channels.registry.channel_manager') as mock_cm:
            mock_cm.get_channel_instance.return_value = None
            sent = scheduler._action_user_notification(config)
            assert sent is False
            mock_cm.get_channel_instance.assert_called_once_with('ch-dead')

    def test_action_user_notification_channel_not_running(self):
        from backend.scheduler import scheduler
        config = {
            'channel_id': 'ch-stopped',
            'external_user_id': 'user-42',
            'message': 'Ping!',
        }
        with patch('backend.channels.registry.channel_manager') as mock_cm:
            mock_channel = MagicMock()
            mock_channel.is_running = False
            mock_cm.get_channel_instance.return_value = mock_channel
            sent = scheduler._action_user_notification(config)
            assert sent is False
            mock_channel.send_message.assert_not_called()

    def test_execute_action_user_notification_creates_log(self):
        from backend.scheduler import scheduler
        s = self._create_schedule(
            'user_notification',
            {'channel_id': 'ch-1', 'external_user_id': 'u99', 'message': 'Test push'},
        )
        with patch('backend.channels.registry.channel_manager') as mock_cm:
            mock_channel = MagicMock()
            mock_channel.is_running = True
